...
| Focused audit GREEN | `npm test -- --runInBand src/ui/darkOnlyVisualSystem.test.ts src/ui/appWideColorSystemContract.test.ts src/entry/residualUtilityAppearanceContract.test.ts src/achievements/trophyCaseAppearanceContract.test.ts src/ui/Toast.stateContract.test.ts src/ui/GlassTabBar.test.ts` | Passed: 6 suites, 46 tests |
| Home/Menu contrast GREEN | `npm test -- --runInBand src/navigation/menuAppearanceContract.test.ts src/navigation/financeRouteRemovalContract.test.ts` | Passed: 2 suites, 21 tests; bright-endpoint text ≥4.5:1 and meaningful icons ≥3:1 |
| Full Jest suite | `npm test -- --runInBand` | Passed: 275 suites, 3,216 tests, 1 snapshot |
