...
describe('immutable R2 uploads', () => {
  ...
  test('fails closed when a share response does not confirm server-side storage', async () => {
    const mediaRef = `r2://share-cards/${memberId}/${sha256}.png`;
    invoke.mockResolvedValue({
      data: { uploadUrl: 'https://uploads.example/legacy-share', mediaRef },
      error: null,
    } as never);
    global.fetch = jest.fn() as unknown as typeof fetch;

    await expect(uploadToR2WithDigest('AQID', 'share', 'png', { operationId }))
      .rejects.toThrow('Could not complete the upload.');
    expect(global.fetch).not.toHaveBeenCalled();
  });

  test('reports a failed presigned non-share PUT without reading its body', async () => {
