...
function renderVault(overrides: VaultOverrides = {}) {
  ...
  return { renderer, choosePhoto, savePhoto, onSaved };
}
...
describe('ProgressPhotoVault', () => {
  ...
  test('drops capture and save completions after an account switch', async () => {
    ...
    const { renderer, savePhoto, onSaved } = renderVault({ choosePhoto });
    act(() => renderer.root.findByProps({ accessibilityLabel: 'Take selfie' }).props.onPress());
    await act(async () => renderer.update(
      <ProgressPhotoVault photos={[]} expectedOwnerId="owner-b" choosePhoto={choosePhoto} savePhoto={savePhoto} now={() => NOW} onSaved={onSaved} />,
    ));
    ...
    await act(async () => renderer.update(
      <ProgressPhotoVault photos={[]} expectedOwnerId="owner-a" choosePhoto={immediate} savePhoto={savePhoto} now={() => NOW} onSaved={onSaved} />,
    ));
    await act(async () => renderer.root.findByProps({ accessibilityLabel: 'Take selfie' }).props.onPress());
    act(() => renderer.root.findByProps({ accessibilityLabel: 'Save private progress photo' }).props.onPress());
    await act(async () => renderer.update(
      <ProgressPhotoVault photos={[]} expectedOwnerId="owner-b" choosePhoto={immediate} savePhoto={savePhoto} now={() => NOW} onSaved={onSaved} />,
    ));
  ...
  test('renders only earliest Before and newest Latest metadata from a bounded preview', () => {
    ...
    const { renderer } = renderVault({ photos });
    const previewLabels = new Set(renderer.root.findAll((node) => /^Private (Before|Latest) progress photo/.test(node.props.accessibilityLabel ?? ''))
  ...
  test('gives actions and modal fields 48-point targets with wrapping controls', async () => {
    ...
    await act(async () => renderer.root.findByProps({ accessibilityLabel: 'Take selfie' }).props.onPress());
    for (const label of ['Progress photo date', 'Cancel private photo', 'Save private progress photo']) {
      const control = renderer.root.findByProps({ accessibilityLabel: label });
      const style = typeof control.props.style === 'function' ? control.props.style({ pressed: false }) : control.props.style;
      expect(StyleSheet.flatten(style).minHeight).toBeGreaterThanOrEqual(48);
    }
    expect(renderer.root.findByType(TextInput).props.maxLength).toBe(10);
  });
});
