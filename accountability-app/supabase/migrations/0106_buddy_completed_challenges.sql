...
-- does NOT remediate that historical privacy exposure. Migration 0108 performs
...
-- operator must provide reviewed zero-legacy-client evidence before 0108 can run.
...
begin
  ...
  if p_expected_owner is null or p_expected_owner <> v_user then
    raise exception 'Account changed. Try again.' using errcode = '42501';
  end if;
...
comment on function public.join_challenge(uuid, integer, uuid) is
  'Replay-safe owner-bound enrollment using database time and a server-owned joined_at, limited to the active challenge window.';
