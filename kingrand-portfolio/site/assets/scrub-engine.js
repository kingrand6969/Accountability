...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    if (s.intro) c.classList.add('sw-copy--intro');
    c.innerHTML =
      `<span class="sw-copy__num">${pad(i + 1)} / ${pad(N)}</span>` +
      (s.photo ? `<img class="sw-copy__photo" src="${esc(s.photo)}" alt="" onerror="this.remove()">` : '') +
      (s.eyebrow ? `<span class="sw-copy__eyebrow">${esc(s.eyebrow)}</span>` : '') +
      (s.title ? `<h2 class="sw-copy__title">${esc(s.title)}</h2>` : '') +
      (s.body ? `<p class="sw-copy__body">${esc(s.body)}</p>` : '') +
      (s.detail && s.detail.length ? `<ul class="sw-copy__detail">${s.detail.map(d => `<li>${esc(d)}</li>`).join('')}</ul>` : '') +
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (i === 0) cop = after ? 0 : smooth(1 - pr / 0.62);            // greets on landing
        ...
        const past = (y - seg.end) / (0.9 * vh);                       // …then hands off to the
        if (past > 0) cop *= smooth(1 - past);                         // in-flow content below
      }
    ...
    // Past the end of the flight the fixed chrome yields to the in-flow content below.
    const pastEnd = clamp((y - SEGMENTS[NSEG - 1].end) / (0.5 * vh));
    route.style.opacity = 1 - pastEnd;
    route.style.pointerEvents = pastEnd > 0.9 ? 'none' : '';
...
function injectCSS() {
  ...
  .sw-copy{position:absolute;left:clamp(18px,5vw,64px);top:clamp(84px,15vh,150px);width:min(42vw,460px);opacity:0;will-change:opacity,transform;}
  ...
  .sw-copy__photo{width:76px;height:76px;border-radius:50%;object-fit:cover;display:block;margin-bottom:14px;border:2px solid color-mix(in srgb,var(--sw-accent) 65%,transparent);box-shadow:0 8px 24px color-mix(in srgb,var(--sw-bg) 65%,transparent);}
  .sw-copy--intro .sw-copy__photo{width:132px;height:132px;border-width:3px;}
  .sw-copy--intro .sw-copy__detail{font-size:clamp(.92rem,1.1vw,1.02rem);max-width:52ch;}
  .sw-copy--intro .sw-copy__detail li{margin-top:9px;}
  .sw-copy__detail{margin:16px 0 0;padding:0;list-style:none;font-size:clamp(.86rem,1vw,.96rem);line-height:1.5;color:color-mix(in srgb,var(--sw-ink) 84%,var(--sw-ink-soft));max-width:44ch;}
  .sw-copy__detail li{position:relative;padding-left:16px;margin-top:7px;text-shadow:0 1px 10px color-mix(in srgb,var(--sw-bg) 92%,transparent);}
  .sw-copy__detail li::before{content:"";position:absolute;left:0;top:.52em;width:7px;height:7px;border-radius:2px;background:var(--sw-accent);}
  ...
  @media (max-width:860px){
    .sw-nav{display:none;}
    .sw-copylayer::before{width:100%;height:64%;top:0;bottom:auto;background:linear-gradient(180deg,var(--sw-bg) 6%,color-mix(in srgb,var(--sw-bg) 70%,transparent) 48%,transparent 100%);}
    ...
    .sw-copy{left:clamp(18px,5vw,64px);right:clamp(18px,5vw,64px);bottom:auto;top:calc(clamp(64px,10dvh,104px) + env(safe-area-inset-top));transform:none;width:auto;max-width:560px;box-sizing:border-box;}
    .sw-copy *{overflow-wrap:break-word;min-width:0;}
    .sw-copy__detail{font-size:.82rem;margin-top:10px;} .sw-copy__detail li{margin-top:4px;}
    .sw-copy__photo{width:56px;height:56px;margin-bottom:8px;}
    .sw-copy--intro .sw-copy__photo{width:108px;height:108px;margin-bottom:12px;}
    .sw-copy--intro .sw-copy__detail{font-size:.92rem;}
